...
function _DA_Logic_GetAuraListForType(typeKey)
    if not DoiteAurasDB or not DoiteAurasDB.spells then return nil end
    if not DoiteEdit_CurrentKey then return nil end
...
local function _EvalRpn(rpn)
    ...
    while i <= n do
        ...
        elseif tok == "AND" or tok == "OR" then
            ...
            local b = st[sb]; st[sb] = nil
            local a = st[sb - 1]; st[sb - 1] = nil
...
local function DoiteAuraLogic_UpdatePreview()
    if not DoiteAuraLogicFrame or not DoiteAuraLogic_CurrentType then return end
...
local function DoiteAuraLogic_BuildRow(frame, index)
    ...
    row.parenOpen:SetWidth(18); row.parenOpen:SetHeight(18)
    ...
    row.parenClose:SetWidth(18); row.parenClose:SetHeight(18)
    ...
    row.andCB:SetWidth(18); row.andCB:SetHeight(18)
    ...
    row.orCB:SetWidth(18); row.orCB:SetHeight(18)
    ...
    row.parenOpen:SetScript("OnClick", function()
        if not DoiteAuraLogic_CurrentType then return end
        local list = _DA_Logic_GetAuraListForType(DoiteAuraLogic_CurrentType)
        if not list or not row._index or not list[row._index] then return end
...
function DoiteLogic.OpenAuraLogicEditor(typeKey)
    ...
    if not DoiteAuraLogicFrame then
        ...
        f.okButton:SetWidth(80); f.okButton:SetHeight(20)
        ...
        f.cancelButton:SetWidth(80); f.cancelButton:SetHeight(20)
