...
local function ComputeGroupLayout(entries, groupName)
    ...
    if vn == 0 then
        ...
        while j <= m do
            local e = entries[j]
            if e then e._computedPos = nil end
            j = j + 1
        end
        return {}
    end
...
function DoiteGroup.RequestReflow()
    ...
    if DoiteGroup._reflowQueued then return end
    DoiteGroup._reflowQueued = 1
    _watch:SetScript("OnUpdate", _RunReflowOnce)
end
...
local function _IsKeyGrouped(key)
    local d
    if DoiteDB and DoiteDB.icons then d = DoiteDB.icons[key] end
    if not d and DoiteAurasDB and DoiteAurasDB.spells then d = DoiteAurasDB.spells[key] end
    local g = d and d.group
    return g and g ~= "" and g ~= "no"
end

local function _HookApplyVisualsIfPresent()
    if DoiteGroup._applyVisualsHooked then return end
    if type(DoiteConditions) ~= "table" then return end
    if type(DoiteConditions.ApplyVisuals) ~= "function" then return end
